...
public class Player : MonoBehaviour 
{
	...
	static public GameObject camera, gunCamera, player, invisCamera, aim, smallAim, skin, aim2, smallAim2, aimParent;
	...
	static public void Create()
	{
		...
		aim.transform.parent = aimParent.transform; //camera.transform;//VRMode != Game.VRMode.NONE ? lens[0].transform : camera.transform;
		aim.transform.localEulerAngles = Vector3.right * (-90f);
		aim.transform.localPosition = Vector3.forward * 0.0002f;//Vector3.forward * (camera.GetComponent<Camera>().nearClipPlane + 0.0002f);
		aim.GetComponent<Renderer>().material.shader = Shader.Find("InverseColor");
		aim.AddComponent<Animation>();
		aim.layer = VRMode != Game.VRMode.NONE ? LayerMask.NameToLayer("Aim") : LayerMask.NameToLayer("Ignore Raycast");

		GameObject aim2 = CustomObject.Circle(0.0018f, Obj.Colour.WHITE, false, 16); //30
		aim2.transform.parent = aimParent.transform;//camera.transform;//VRMode != Game.VRMode.NONE ? lens[0].transform : camera.transform;
		aim2.transform.localEulerAngles = Vector3.right * (-90f);
		aim2.transform.localScale = Vector3.zero;
		aim2.transform.localPosition = Vector3.forward * 0.0001f;//(camera.GetComponent<Camera>().nearClipPlane + 0.0001f);
	...
	void Update ()
	{
		...
		if (VRMode == Game.VRMode.FIBRUM)
		{
			...
			{
				//0.0001f
				aimParent.transform.localPosition = Vector3.forward * (hit.distance);
				//smallAim.transform.localPosition = Vector3.forward * (hit.distance - 0.002f);

				aimParent.transform.localScale = Vector3.one * hit.distance * 0.02f / 0.0036f;
		...
		if(Level.current.Index != 0 && !InZeroRoom)
		{
			if( Game.IsInputEscape() )
			{
				...
				if (VRMode == Game.VRMode.FIBRUM)
					GameObject.FindObjectOfType<Level_0> ().binary = 1;
			}
