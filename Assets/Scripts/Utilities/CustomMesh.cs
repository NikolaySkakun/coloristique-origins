...
{
	static public Mesh Circle(int vertsCount = 50, float radius = 0.5f)
	{
		Mesh mesh = new Mesh();
	...
	static public void OptimizeMesh(Mesh mesh)
	{
		List<int> cloneTris = new List<int> ();
		List<int> outTris = new List<int> ();


		List<Vector3> outVerts = new List<Vector3> ();
		List<int> cloneVerts = new List<int> ();
		...
		{
			bool next = false;
			for (int j = 0; j < cloneVerts.Count; ++j)
			{
				if (i == cloneVerts [j])
				{
					next = true;
			...
			bool includeToOut = true;
			for (int u = i + 1; u < verts.Length; ++u)
			{
				if (verts [i] == verts [u])
				{
...
//							maxt = tris [k];
					...
					//Debug.LogError ("V: " + verts [i].ToString() + "   i: " + i.ToString() + "   u: " + u.ToString());
					cloneVerts.Add (u);
					//includeToOut = false;
				}
					//++c;
...
//			}
		...
		int index = 0;
		int[] subt = new int [tris.Length];

		for (int i = 0; i < verts.Length; ++i)
		{
			if (cloneVerts [index] == i)
			{
		...
		for (int i = 0; i < tris.Length; ++i)
		{
			...
			//if (tris [i] >= outVerts.Count)
			//	tris [i] = outVerts.Count - 1;
		}

		Debug.LogError ("t: " + maxt.ToString());
		Debug.LogError ("v: " + outVerts.Count.ToString ());
//		for (int i = 0; i < tris.Length; ++i)
//		{
//			if(tris[i] == 44)
//				Debug.LogError("sdsdsdsdsd"
//		}

//		for (int i = cloneVerts.Count - 1; i >= 0; --i)
//		{
//			for (int u = 0; u < tris.Length; ++u)
//			{
//				if(tris[u] > tris[cloneVerts[i]])
//					--tris[u];
//			}
//		}

		//Debug.LogError (maxt);
//		Debug.LogError (cloneVerts.Count);

//		int index = 0;
//
//		for (int i = 0; i < verts.Length; ++i)
//		{
//			if (i != cloneVerts [index])
//			{
//				outVerts.Add (verts [i]);
//			}
//			else
...
//		for (int i = 0; i < tris.Length - 1; i += 3) // all triangles
//		{
//			for (int u = i + 3; u < tris.Length; u += 3) // a
//			{
//				if (tris [i] == tris [u]) // if t[a] == t'[a]
//				{
//					for (int j = i + 4; j < tris.Length; j += 3) // b
//					{
//						if (tris [i + 1] == tris [j]) // if t[b] == t'[b]
//						{
//							for (int k = i + 5; k < tris.Length; k += 3) // c
//							{
//								if (tris [i + 2] == tris [k]) // if t[c] == t'[c]
//								{
//									cloneTris.Add (i);
//								}
//							}
...
//
//		
//		if (cloneTris.Count > 0)
//		{
//			for (int i = 0; i < tris.Length; i += 3)
//			{
//				if (cloneTris [index] != i)
//				{
//					outTris.Add (tris [i]);
//					outTris.Add (tris [i + 1]);
//					outTris.Add (tris [i + 2]);
//				} else
//				{
//					++index;
//					continue;
...
0, 2, 1
	...
	static public Mesh Semicircle(int vertsCount = 25, float radius = 0.5f)
	{
		Mesh mesh = Circle(vertsCount*2, radius);
